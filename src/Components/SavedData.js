                ...
                <div>
                    {fetchedData.length > 0 ? (
                        <div className="flex flex-row flex-wrap ">
                            {fetchedData.map((id) => (
                                <div className="flex flex-row w-1/2 mt-2 text-lg">
                                    <p
                                    ...
                                    <button onClick={()=>deleteElement(id)}
                                            className="border-2 p-2 w-[15%] mr-2 rounded-r-lg backdrop-blur"><img src="https://img.icons8.com/?size=100&id=11705&format=png&color=FFFFFF"/></button>
                                </div>
                            ))}
                        </div>
                    ) : (
